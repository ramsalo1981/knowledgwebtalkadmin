...
class Edit extends Component {
  ...
  handleCtodId(e) {
    ...
    this.setState({
      Paragraphes: this.state.Articles[e.target.value].articleParagraphs,
    });
    this.forceUpdate();
  }
  ...
  render() {
      ...
      <div className="edited">
        ...
        {this.state.Paragraphes.map((paragraph) => {
          return (
            <EditedPopup
              paragraph={paragraph}
              key={paragraph.articleParagraphId}
            />
          );
        })}
      </div>
    ...
    return <div className="parentEdited layout-size1of1">{renderProducts}</div>;
  }
}
